...
public class SubcategoryInventory {
	...
	itemStackBuilder.setDisplayName("§e" + String.valueOf(item.getType().name().charAt(0)).toUpperCase()
		+ item.getType().name().toLowerCase().substring(1).replace("_", " "));

	itemStackBuilder.setLore("§eOstohinta: " + format.format(buyOne) + "/kpl",
		"§eMyyntihinta: " + format.format(sellOne) + "/kpl");
